...
5. ```prompts_making.ipynb``` - эта тетрадка делает вообще все варианты промптов которые используются в эксперименте описанном в статье. мы используем часть из них для дальнейшей работы. в качестве дополнительного результата, в ней приводится достраивание промптов указанием на то, есть ли наблюдение за цепочкой размышлений модели (это наша идея для дополнения). на вход принимает ```sql-console-for-cais-mmlu.json``` и/или ```gpqa.json```, на выходе в переменные записываются листы, которые находятся в корректном формате подачи в модель для эксперимента
